...
- 通用版默认选择器只是起点；真实项目需要配置租户 URL、目标群聊名称、选择器、滚动范围和日期范围。
...
node scripts/crawl_feishu_threads.mjs \
  ...
  --chat-name "$FEISHU_TARGET_CHAT_NAME" \
  --max-threads 20 \
...
- `report.json`：抓取报告，包含目标群聊选择结果、实际选择器、新增和更新数量

如果 `chat_selection.selected` 为 `false`、`visible_threads` 为 0，或 `captured_threads` 为 0，先按 `docs/customizing-feishu-crawler.md` 调整目标群聊名称和选择器，不要假设脚本已经适配当前租户。
